...
def user_update(user_id=0):
    ...
    return flask.render_template(
        ...
        api_url=flask.url_for(
            'api.user', user_key=user_db.key.urlsafe()) if user_db.key else ''
    )
...
def user_reset(token=None):
    ...
    if form.validate_on_submit():
        user_db.password_hash = util.password_hash(
            user_db, form.new_password.data)
        user_db.token = util.uuid()
        user_db.verified = True
        user_db.put()
...
class UserMergeForm(wtf.Form):
    ...
    user_keys = wtforms.StringField(
        'User Keys', [wtforms.validators.required()])
